...
export type Mirage3Prediction = "wife" | "mom" | "money";

interface Mirage3BettingPanelProps {
  balance: number;
  onPlaceBet: (prediction: Mirage3Prediction, amount: number) => void;
  disabled?: boolean;
  onLoginClick?: () => void;
  phase: "deciding" | "conclusion" | "result" | "publish";
  currentBet: { prediction: Mirage3Prediction; amount: number } | null;
  onVideoEnded: () => void;
  lastResult: { won: boolean; amount: number; payout: number } | null;
  onPosted: () => void;
  showPostedConfirmation: boolean;
}

const PRESET_AMOUNTS = [10, 50, 100, 250];
const RESULT_DISPLAY_DURATION = 3000;

export function Mirage3BettingPanel({
  balance,
  onPlaceBet,
  disabled = false,
  onLoginClick,
...
}: Mirage3BettingPanelProps) {
  const [amount, setAmount] = useState<number>(50);
  const [selectedPrediction, setSelectedPrediction] = useState<Mirage3Prediction | null>(null);
  ...
  useEffect(() => {
    if (phase === "result" && lastResult) {
      ...
      const timer = setTimeout(() => {
        setShowResult(false);
        onVideoEnded(); // This will transition to publish phase
  ...
  const handlePredictionClick = (prediction: Mirage3Prediction) => {
